...
| `mu-plugins/rg-stock-colibri-drawer.php` | 1.0.0 | [ddc398f](https://github.com/rivegosh/concierge-rivegosh/commit/ddc398f) | [#109](https://github.com/rivegosh/concierge-rivegosh/issues/109) | Single `remove_action('wp_footer', 'rivegosh_custom_drawer_v43', 99998)` so Colibri's native offcanvas is used on mobile (stock behavior). Replaces the disabled `rg-drawer-override.php`. Zero hardcoded menu items — Daniel's menu 86 fully controls nav. |

**🚫 DISABLED 2026-04-23 (do NOT re-enable without reading Issue #109):**
- `rg-header-menu-lock.php` → `.disabled-2026-04-23` on server — had race-condition creating duplicate menu items + blocking Daniel's edits. Issue [#78](https://github.com/rivegosh/concierge-rivegosh/issues/78) reversed.
- `rg-drawer-override.php` → `.disabled-2026-04-23` on server — hardcoded mobile drawer fallback blocked Daniel's nav edits.
- `rg-v52-card-icons.php` → `.disabled-2026-04-23` on server — forced menu locations + replaced mobile nav with 8 hardcoded items.
...
| `mu-plugins/rg-checkout-payment-sizing.php` | 1.0.0 | [5182f62](https://github.com/rivegosh/concierge-rivegosh/commit/5182f62) | — | Payment section input + typography: radio/checkbox scale(1.5) with margin compensation (transform doesn't affect layout flow). Payment method labels ("Stripe", "card") 15px. T&C checkbox text 14px (+1pt, must remain un-ticked by default — GDPR active consent). Privacy notice 13px (−3pt, dimmer opacity). Priority 100009. |
| `mu-plugins/rg-checkout-myspace-text.php` | 1.0.0 | [1d0d47a](https://github.com/rivegosh/concierge-rivegosh/commit/1d0d47a) | — | My Space section free-text content in order review: 16px (+2pt from hierarchy plugin's 14px base), line-height 1.25 (tighter than 1.5). Targets `dl.variation dt:last-of-type ~ dd p` — the last dt is always MY SPACE. Priority 100010. |
| `mu-plugins/rg-cart-order-hierarchy.php` | 1.0.0 | [816bc63](https://github.com/rivegosh/concierge-rivegosh/commit/816bc63) | — | Cart page (page-id-14) typography hierarchy. 5 fixes: (1) `dl.variation dt` float:none killed "Appointment Info"+"Local Time" collision; 11px uppercase champagne. (2) 9 inline `<b>`/`<strong>` Amelia field labels (Local Time, Client Time, service, VIP Driver, Total Number of People, Custom Fields, Your Airport Arrival/Departure, Special request, How many suitcase) were rendering at 20px/700 — normalized to 11px/600/uppercase (incidentally fixes Amelia's lowercase "service:"). (3) MY SPACE free text 15px. (4) `<a class="wcfm_dashboard_item_title">` wrapping "Rive gosh" neutralized to plain value. (5) `td.product-price` + `td.product-subtotal` 16px→19px champagne bold vertical-align:top. Priority 100010. Runtime-verified: 10/10 computed-style assertions + 0 contrast violations. |
| `mu-plugins/rg-order-received-success.php` | 1.0.0 | TBD | — | Order-received page (/checkout/order-received/) success banner + order-details layout. (1) `woocommerce_thankyou_order_received_text` filter replaces plain "Thank you" with 3-line structured notice: **SUCCESS** (32px green uppercase) + "Thank you. Your order has been received." (18px white) + "Your chauffeur service will confirm your booking by email shortly." (14px italic champagne). (2) Green accent (#5fb88a) left border + tinted bg on `.woocommerce-notice`. (3) Shows in BOTH stripped (no key) and full views — fixes previously-blank stripped view. (4) Order-details table uses raw `<br>` spam + `<hr>` (NOT `dl.variation` like cart) — `br + br { display:none }` collapses 5-BR runs that created ~120px gap after "Appointment Info". (5) Totals column 140px + vertical-align:top so "1:00 am" no longer wraps. (6) Inline `<strong>`/`<b>` field labels 11px uppercase champagne (except `strong.product-quantity` which stays 14px body size). (7) tfoot `white-space: normal` to accommodate "Credit / Debit Card" text. Scoped `body.woocommerce-order-received` — zero bleed to /checkout/. Priority 100011. |
