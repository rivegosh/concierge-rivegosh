...
| `mu-plugins/rg-service-detail-fix.php` | 1.0.0 | [83ce89c](https://github.com/rivegosh/concierge-rivegosh/commit/83ce89c) | — | Two fixes for Amelia service detail (.am-fcis): (1) Book Now button — sealed §8 of rg-catalog-luxury-reskin.php beats §6 champagne fill in specificity tie (both 1,3,0; §8 is later). Fix: (1,4,0) selector + priority 100005 restores champagne gold. (2) Gallery hero black void — min-height 180px safety net + img fallback for zero-height first-render race condition. Priority 100005. |
| `mu-plugins/rg-checkout-select2-dropdown.php` | 1.0.0 | TBD | — | Dark luxury skin for select2 open dropdown on /checkout/ (page-id-15). rg-checkout-coupon-fix.php §4 only styles the collapsed trigger. This plugin targets the teleported dropdown panel (.select2-dropdown), search field, option items (default/highlighted/selected states). select2 appends to body directly → body.woocommerce-checkout is the correct ancestor scope. Priority 100006. |
